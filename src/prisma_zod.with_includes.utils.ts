...
export function with_includes (schemas: any, schemaObj: Record<any, any>, includes: Record<string, any>) {
  ...
  Object.entries(includes).map(([key, el]) => {
    ...
    const [name_schema1] = obj_first_property(data.simple_related_zod_schema[key]?.shape) as [any, any]
    if(typeof el === "boolean"){
      new_schema[key] = data.simple_related_zod_schema[key].array ? schemas[name_schema1].array() : schemas[name_schema1]
    }else if(typeof el === "object"){
      new_schema[key] = with_includes(schemas, { [name_schema1]: schemas[name_schema1] }, includes[key].include )
    }
  })
...
const get_related_shape = ( schemas: any, name_schema: any ) => {
  ...
  return {
    core_zod_schema,
    full_shape, core_shape, related_shape, simple_related_zod_schema
  }
}