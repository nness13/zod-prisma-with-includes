...
const useModelNames = ({
  ...
  return {
    ...
    simpleRelatedModelName: name => formatModelName(relationModel === 'default' ? `Simple${name.toString()}` : `SimpleRelated${name.toString()}`)
  };
};
  ...
  if (config.relationModel !== false && relationFields.length > 0) {
    ...
    if (filteredFields.length > 0) {
      importList.push({
        ...
        namedImports: Array.from(new Set(filteredFields.flatMap(f => [modelName(f.type), `Complete${f.type}`, relatedModelName(f.type)])))
      });
    }
  }
...
const generateSimpleRelatedSchemaForModel = (model, sourceFile, config, _prismaOptions) => {
  const {
    modelName,
    ...
    declarations: [{
      ...
      initializer(writer) {
        writer.inlineBlock(() => {
          relationFields.forEach(field => {
            writeArray(writer, getJSDocs(field.documentation));
            writer.write(`${field.name}: {shape: { ${modelName(field.type)} }, array: ${field.isList ? 'true' : 'false'}}`).write(',').newLine();
          });
        });
      }
    }]
...
const populateModelFile = (model, sourceFile, config, prismaOptions) => {
  ...
  if (needsRelatedModel(model, config)) {
    ...
    generateSimpleRelatedSchemaForModel(model, sourceFile, config);
  }
};
...
function with_includes(schemas, schemaObj, includes) {
...
const get_related_shape = (schemas, name_schema) => {
  ...
  const core_shape = ___default["default"].pick(full_shape, core_zod_schema_keys);
  const related_shape = ___default["default"].omit(full_shape, core_zod_schema_keys);
  return {
    core_zod_schema,
    full_shape,
    core_shape,
...
exports.with_includes = with_includes;
//# sourceMappingURL=zod-prisma-with-includes.cjs.development.js.map
